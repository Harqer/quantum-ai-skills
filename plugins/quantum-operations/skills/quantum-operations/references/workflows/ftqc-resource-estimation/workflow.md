...
Report logical resources, physical data/QEC qubits, factory footprint and throughput, decoder/controller compute and memory, baseline schedule depth, stall sources, wall-clock runtime, and the composed error budget as separate fields. For toric-code candidates, include the cost of realizing periodic seams on the target hardware—native wrap-around couplers, routing, movement, teleportation/modular links, or long-range interactions—rather than pricing only the abstract 2L^2 data-qubit lattice. For qutrit/qudit candidates, distinguish information-capacity compression from executable resource reduction: charge dimension-dependent pulse count, control serialization, level-dependent coherence, SPAM, leakage, multiqudit entangling fidelity, reset/readout time, oscillator energy for bosonic codes, and any unavailable provider controls. Do not replace N qubits by ceil(N/log2(d)) carriers unless the mapped workload and demonstrated gate set realize that packing. Sweep physical error rates and timings, code parameters, factory choices, decoder configurations, feed-forward latency, routing/layout, dimension d, and slowdown targets, then preserve the Pareto frontier instead of collapsing the design space into one opaque score.
